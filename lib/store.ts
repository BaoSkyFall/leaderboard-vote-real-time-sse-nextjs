...
const K_ONLINE = "presence:online"; // sorted set userId -> last-seen epoch ms
const kTs = (candidateId: string) => `votes:ts:${candidateId}`; // list of epoch ms

const ONLINE_TTL_MS = 10_000; // a ping is considered "online" for this long
...
interface MemoryStore {
  ...
  online: Map<string, number>; // userId -> last-seen epoch ms
}
...
// ---------------------------------------------------------------------------
// Presence (best-effort online counter)
// ---------------------------------------------------------------------------
export async function ping(userId: string): Promise<void> {
  ...
  if (redis) {
    ...
    // Trim stale entries opportunistically.
    await redis.zremrangebyscore(K_ONLINE, 0, now - ONLINE_TTL_MS);
  } else {
...
export async function getOnlineCount(): Promise<number> {
  ...
  for (const [uid, seen] of mem.online) {
    if (now - seen <= ONLINE_TTL_MS) count++;
